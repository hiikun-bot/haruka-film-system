...
router.get('/bug-reports', requireAuth, async (req, res) => {
  try {
    ...
    let q = supabase
      .from('bug_reports')
      .select('id, is_anonymous, title, description, url, screen_label, severity, is_urgent, status, assignee_user_id, created_at, updated_at, resolved_at, reporter_user_id, screenshot_data_url, reporter:reporter_user_id ( id, full_name, nickname, avatar_url ), assignee:assignee_user_id ( id, full_name, nickname, avatar_url )')
      .order('created_at', { ascending: false });
    if (status) q = q.eq('status', status);
