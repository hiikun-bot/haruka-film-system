...
// （applies_from/applies_to・停止/再開）を無視するため、単価変更（旧グループ停止＋
// 新グループ作成）後の納品済みクリエイティブが旧単価のまま計上されてしまう問題を是正。
// ─────────────────────────────────────────────────────────────────────────

// 集計対象月の JST 月末日を 'YYYY-MM-DD' 文字列で返す。
// applies_from/applies_to は DATE ('YYYY-MM-DD') 文字列なので、この文字列と辞書順比較する。
// month は 1 始まり。Date.UTC(year, month, 0) は当月末日（翌月 0 日目）を指す。
function jstMonthEndStr(year, month) {
  const lastDay = new Date(Date.UTC(year, month, 0)).getUTCDate();
  return `${year}-${String(month).padStart(2, '0')}-${String(lastDay).padStart(2, '0')}`;
}

// project_estimate_lines を「単価解決に必要な列（category_id/rank/applies_from/applies_to 付き）」で
// 取得する。applies_from/applies_to（ADR 025）や category_id/rank が未適用の環境では旧 select に
...
async function loadEstimateLinesForPricing(filter, tag) {
  ...
  let res = await applyFilter(supabase.from('project_estimate_lines').select(withApplies));
  if (res.error) {
    // applies_from/applies_to（または category_id/rank）が未適用の環境 → 旧 select で再試行。
    // フォールバックした line には category_id が入らないため resolveEffectiveLineId は元 line を返す。
    console.warn(`[${tag}] lines load with applies columns failed, fallback to legacy select:`, res.error.message);
    res = await applyFilter(supabase.from('project_estimate_lines').select(legacy));
    if (res.error) {
      console.warn(`[${tag}] lines legacy load failed:`, res.error.message);
      return [];
    }
  }
  return res.data || [];
...
// 元 line が属する (project_id, category_id, rank) の成果物グループ群から、締め月末時点で
// 有効な line の id を1つ選ぶ（ADR 030）。
//   - applies_from IS NULL OR applies_from <= 月末
//   - applies_to   IS NULL OR applies_to   >= 月末
...
// 🎯 マイゴール（完全個人の目標・タスク管理）
// ------------------------------------------------------------
// 背景: 個人の目標（例: 法人設立）とタスク（大分類→中分類→タスクの3階層）を
//       本人だけが管理するページ。進捗率・ストリーク・今週完了数の集計は
//       サーバーでは行わず、クライアント側（utils/personal-goals.js を /js/ で配信）で算出する。
// テーブル: personal_goals / personal_tasks（migrations/2026-08-21_personal_goals.sql）
// 権限: requireAuth のみ（permission key 無し・全ロール利用可）。
// ⚠️ 完全個人領域（ADR 032）: 本人（req.user.id）以外は admin であっても一切読めない・書けない。
//    全クエリで req.user.id を条件に使い、user_id パラメータは受け付けない。
...
router.patch('/personal-goals/:id', requireAuth, async (req, res) => {
  ...
  if (status !== undefined) {
    if (!PG_GOAL_STATUSES.has(status)) {
      return res.status(400).json({ error: `status は ${[...PG_GOAL_STATUSES].join(' / ')} のいずれかで指定してください` });
    }
    update.status = status;
    update.achieved_at = status === 'achieved' ? new Date().toISOString() : null;
  }
  ...
  const { data, error } = await supabase
    ...
    .update(update)
    .eq('id', req.params.id)
    .eq('user_id', req.user.id) // IDOR防止: 他人の目標IDを指定しても 0行 → 404
    .select()
    .maybeSingle();
  if (error) {
    if (isMissingPersonalGoalsTable(error)) return personalGoalsMigrationHint(res);
  ...
  if (!data) return res.status(404).json({ error: '目標が見つかりません' });
  res.json(data);
});

// DELETE /personal-goals/:id — 目標の削除（紐付くタスクは FK ON DELETE SET NULL で残る）
router.delete('/personal-goals/:id', requireAuth, async (req, res) => {
  const { data, error } = await supabase
    .from('personal_goals')
    .delete()
    .eq('id', req.params.id)
    .eq('user_id', req.user.id) // IDOR防止: 他人の目標は削除不可（0行 → 404）
    .select('id')
    .maybeSingle();
  if (error) {
    if (isMissingPersonalGoalsTable(error)) return personalGoalsMigrationHint(res);
...
// goal_id が「本人の目標」を指しているか検証（他人の goal_id への紐付けを拒否）
async function pgAssertOwnGoal(goalId, userId) {
  const { data, error } = await supabase
    .from('personal_goals')
    .select('id')
    .eq('id', goalId)
...
router.post('/personal-tasks', requireAuth, async (req, res) => {
  ...
  const { data, error } = await supabase.from('personal_tasks').insert(insert).select().single();
  if (error) {
    if (isMissingPersonalGoalsTable(error)) return personalGoalsMigrationHint(res);
    return res.status(500).json({ error: error.message });
  }
...
router.patch('/personal-tasks/:id', requireAuth, async (req, res) => {
  ...
  if (memo !== undefined) update.memo = pgTrimOrNull(memo);
  if (link_url !== undefined) update.link_url = pgTrimOrNull(link_url);
  ...
  const { data, error } = await supabase
    ...
    .update(update)
    .eq('id', req.params.id)
    .eq('user_id', req.user.id) // IDOR防止: 他人のタスクIDを指定しても 0行 → 404
    .select()
    .maybeSingle();
  if (error) {
    if (isMissingPersonalGoalsTable(error)) return personalGoalsMigrationHint(res);
...
// DELETE /personal-tasks/:id — タスクの削除
router.delete('/personal-tasks/:id', requireAuth, async (req, res) => {
  const { data, error } = await supabase
    .from('personal_tasks')
    .delete()
    .eq('id', req.params.id)
    .eq('user_id', req.user.id) // IDOR防止: 他人のタスクは削除不可（0行 → 404）
    .select('id')
    .maybeSingle();
  if (error) {
    if (isMissingPersonalGoalsTable(error)) return personalGoalsMigrationHint(res);
