...
// ===================================================================
// 新方式: 動画長で分岐するプレビュー生成（preview_* 列に書く）
//
//   - 短尺（< 180秒）: H.264 faststart 版を Drive にアップロードし
//     preview_strategy='h264_faststart' で記録（既存 _processDriveFile 再利用）。
//   - 長尺（>= 180秒）: 60フレーム抽出 → WebP アニメ化して
//     preview_strategy='webp_storyboard' で記録。Drive 全DLを避けるため
//     ffmpeg に -ss / -t を渡して必要なフレーム時刻だけローカルに切り出す。
//
// 既存 generateFaststartForVideoOrg は互換のため残しているが、新規呼び出しは
// 本関数 generatePreviewForVideoOrg を使うこと。
// ===================================================================

const LONG_VIDEO_THRESHOLD_SECONDS = 180; // 3分
...
async function _processLongVideoStoryboard({ sourceDriveFileId, sourceFileName, durationSeconds }) {
  ...
  try {
    ...
    // 抽出区間: 2%地点 〜 98%地点
    const startT = Math.max(0.5, durationSeconds * 0.02);
    const endT   = Math.max(startT + 0.5, durationSeconds * 0.98);
    ...
    const outName = `${baseName}_preview60.webp`.replace(/[^\w.\-]/g, '_');
    const outFp = path.join(tmpDir, outName);
...
// 素材広場 (video_file_organization_tests) 用: 動画長で分岐するプレビュー生成。
...
async function generatePreviewForVideoOrg(arg) {
  ...
  const { data: row, error } = await supabase
    ...
    .select([
      ...
      'preview_drive_file_id', 'preview_status',
      'faststart_drive_file_id', 'faststart_status',
    ].join(', '))
    .eq('id', rowId)
    .maybeSingle();
  if (error || !row) {
  ...
  // processing マーク
  await _updatePreviewRow(rowId, { preview_status: 'processing' });

  // 長さ不明 or 短尺 → H.264 faststart 版を作る
  const isShort = !dur || dur < LONG_VIDEO_THRESHOLD_SECONDS;
  if (isShort) {
    if (!isVideoCandidate(row.mime_type || 'video/mp4', sourceFileName)) {
      await _updatePreviewRow(rowId, {
        preview_status: 'skipped',
    ...
    await _updatePreviewRow(rowId, {
      preview_drive_file_id: result.faststartDriveFileId,
      preview_drive_url:     result.faststartUrl,
      preview_file_size:     result.faststartFileSize,
      preview_mime_type:     'video/mp4',
      preview_strategy:      'h264_faststart',
      preview_status:        'done',
      preview_duration_seconds: dur,
      preview_processed_at:  new Date().toISOString(),
      // 既存 faststart_* 列にも互換のため同じ値を書く
      faststart_drive_file_id: result.faststartDriveFileId,
      faststart_drive_url:     result.faststartUrl,
      faststart_file_size:     result.faststartFileSize,
      faststart_status:        'done',
    ...
    console.log('[preview] done (h264):', {
      rowId, mode: result.mode, outSize: result.faststartFileSize,
    });
    return { ok: true, strategy: 'h264_faststart', mode: result.mode, outSize: result.faststartFileSize };
  }

  // 長尺: WebP 60フレーム
  const result = await _processLongVideoStoryboard({
    sourceDriveFileId: row.drive_file_id,
    sourceFileName:    sourceFileName,
    durationSeconds:   dur,
  });
  ...
  console.log('[preview] done (webp):', {
    rowId, outSize: result.previewFileSize, duration: dur,
  });
