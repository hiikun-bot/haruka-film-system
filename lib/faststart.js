...
//       preset は長尺でも現実的時間で完了させるため medium。画質指標 CRF は据え置き）
...
async function _processDriveFile({ sourceDriveFileId, sourceFileName, sourceMimeType }) {
  ...
  try {
    ...
    // メモリ事故防止: 動画全体を arraybuffer でメモリに載せず、ストリームで
    // ローカルファイルに書き出す。長尺/大容量だと arraybuffer は Railway の
    // メモリ上限を超えて OOM し、faststart_status='failed' になっていた
    // （プレビュー生成側 generatePreviewForVideoOrg と同じ対処に揃える）。
    {
    ...
    await new Promise((resolve, reject) => {
      ...
      if (copyOnly) {
        ...
        cmd.outputOptions([
          '-c:v libx264',
          '-crf 18',
          // preset slow は長尺の再エンコードで時間切れになりやすかった。
          // medium で 2〜3 倍速・画質指標(CRF18)は据え置き（画質劣化ゼロ方針を維持）。
          '-preset medium',
          '-c:a aac',
          '-b:a 192k',
          '-movflags +faststart',
    ...
    // アップロードもメモリに載せない。出力も長尺だと大容量なので
    // readFileSync(全バイトをメモリ) ではなく createReadStream で送る。
    const outSize = fs.statSync(outputFp).size;
    const upRes = await drive.files.create({
      requestBody: { name: outName, parents: [parentFolderId] },
      media: { mimeType: 'video/mp4', body: fs.createReadStream(outputFp) },
...
async function generateFaststart(arg) {
  ...
  // 再生用ホットキャッシュ: faststart 版を Cloudflare R2 へ複製（fire-and-forget）。
  // R2_PLAYBACK_ENABLED=true でなければ完全 skip。予算ガード（10GB無料枠）で複製を
  // 見送ることもある。失敗しても faststart 自体は成功扱い（再生は Drive フォールバック）。
  try {
    const r2 = require('./r2');
    if (r2.isEnabled()) {
      r2.replicateCreativeFileToR2(creativeFileId)
        .catch(err => console.warn('[r2] faststart 後の複製失敗:', creativeFileId, err?.message));
    }
  } catch (e) {
    console.warn('[r2] 複製トリガー失敗:', e.message);
  }
...
function _extractFrameAt({ inputArg, timestamp, outputPath }) {
  return new Promise((resolve, reject) => {
    ...
    ffmpeg()
      ...
      // 長辺 640px に縮小してから抽出する。原本解像度のまま 60 枚抜くと、4K 級の
      // 高解像度素材（例: まるぽんカメラの MOV）では WebP ストーリーボードが 40MB に
      // 達し、Gemini の inline 20MB 上限を超えて解析が skipped になっていた
      // （auto-analyze.js の MAX_INLINE_BYTES ガード）。雰囲気把握用なので 640px で十分。
      // min(640,iw) で原本より小さい素材を無駄に拡大しない。-2 で高さを偶数に揃える。
      .outputOptions(['-vf', "scale='min(640,iw)':-2", '-frames:v', '1', '-q:v', '3', '-y'])
      .on('end', () => resolve())
...
// ==================== 解析用プロキシ（ADR 039 D1） ====================
// Gemini に渡す入力は WebP（アニメーションだと先頭 1 コマしか読まれない）ではなく、
//   (1) 60 コマを 1fps で並べた 60 秒の無音 H.264 動画（Gemini が時系列で全コマを読む）
//   (2) 原本の音声トラック（AAC mono 48kbps。発話内容が名前付け・タグの根拠になる）
// の 2 ファイル。プレビュー WebP と同じ Drive フォルダに置き、再解析時の原本再ダウンロードを避ける。
const PROXY_AUDIO_MAX_SECONDS = 30 * 60;   // 48kbps mono → 30 分で ~11MB（inline 20MB 上限に余裕）
const PROXY_AUDIO_BITRATE = '48k';

function _buildProxyVideoFromFrames({ pattern, outputPath, framerate = 1 }) {
  return new Promise((resolve, reject) => {
    if (!ffmpeg) return reject(new Error('ffmpeg not available'));
    ffmpeg()
      .input(pattern)
      ...
      .outputOptions([
        ...
        '-crf', '28',
        '-pix_fmt', 'yuv420p',
        // 抽出コマは長辺 640px。H.264 は偶数サイズが必要なので念のため丸める
...
// 原本から音声だけを AAC(ADTS) で切り出す。Gemini の対応 MIME は audio/aac。
function _extractProxyAudio({ inputFp, outputPath, maxSeconds }) {
  return new Promise((resolve, reject) => {
    if (!ffmpeg) return reject(new Error('ffmpeg not available'));
    const out = ['-vn', '-ac', '1', '-ar', '24000', '-c:a', 'aac', '-b:a', PROXY_AUDIO_BITRATE, '-f', 'adts', '-y'];
    if (maxSeconds && maxSeconds > 0) out.unshift('-t', String(Math.ceil(maxSeconds)));
    ffmpeg()
      .input(inputFp)
      .outputOptions(out)
      .on('end', () => resolve())
...
// user OAuth → SA フォールバック付きの Drive アップロード（プレビュー WebP と同じ方針）
async function _uploadWithUserFallback({ drive, uploadAsUserDrive, name, mimeType, buffer, parentFolderId }) {
  const mk = () => { const pt = new PassThrough(); pt.end(buffer); return pt; };
  const req = (client, body) => client.files.create({
  ...
  if (uploadAsUserDrive) {
    try {
      const r = await req(uploadAsUserDrive, mk());
      return { id: r.data.id, webViewLink: r.data.webViewLink || null, uploadedAsUser: true, client: uploadAsUserDrive };
    } catch (e) {
      console.warn('[preview] user OAuth upload failed, falling back to SA:', name, e?.message || e);
    }
  }
  const r = await req(drive, mk());
  return { id: r.data.id, webViewLink: r.data.webViewLink || null, uploadedAsUser: false, client: drive };
}

// 60 コマ jpg + 原本から解析用プロキシ 2 本を作って Drive に置く。失敗してもプレビュー本体は止めない。
async function _buildAndUploadAnalysisProxies({
  drive, uploadAsUserDrive, parentFolderId, tmpDir, frameDir, inputFp, baseName, probed, effectiveDuration,
}) {
  const result = { status: 'failed', videoDriveFileId: null, audioDriveFileId: null, audioSeconds: null, hasAudio: false, error: null,
    videoSize: null, audioSize: null };
  try {
    const safeBase = String(baseName || 'analysis').replace(/[^\w.\-]/g, '_');
    const videoFp = path.join(tmpDir, `${safeBase}.analysis.mp4`);
    await _buildProxyVideoFromFrames({ pattern: path.join(frameDir, 'frame_%03d.jpg'), outputPath: videoFp, framerate: 1 });
    const videoBuf = fs.readFileSync(videoFp);
    const up = await _uploadWithUserFallback({
      drive, uploadAsUserDrive, parentFolderId,
      name: `${baseName}.analysis.mp4`, mimeType: 'video/mp4', buffer: videoBuf,
    });
    result.videoDriveFileId = up.id;
    result.videoSize = videoBuf.length;

    const hasAudio = Array.isArray(probed?.streams) && probed.streams.some(st => st?.codec_type === 'audio');
    result.hasAudio = hasAudio;
    if (hasAudio) {
      const audioSeconds = Math.min(Number(effectiveDuration) || 0, PROXY_AUDIO_MAX_SECONDS);
      const audioFp = path.join(tmpDir, `${safeBase}.analysis.aac`);
      try {
        await _extractProxyAudio({ inputFp, outputPath: audioFp, maxSeconds: audioSeconds });
        const audioBuf = fs.readFileSync(audioFp);
        const upA = await _uploadWithUserFallback({
          drive, uploadAsUserDrive, parentFolderId,
          name: `${baseName}.analysis.aac`, mimeType: 'audio/aac', buffer: audioBuf,
        });
        result.audioDriveFileId = upA.id;
        result.audioSeconds = audioSeconds;
        result.audioSize = audioBuf.length;
      } catch (e) {
        // 音声だけ失敗 → 映像のみで解析できるので done 扱い（error は記録）
        console.warn('[preview] proxy audio failed (video-only で続行):', e?.message || e);
        result.error = `audio: ${e?.message || e}`;
      }
    }
    result.status = 'done';
  } catch (e) {
    result.error = e?.message || String(e);
    console.warn('[preview] analysis proxy failed:', result.error);
  }
  return result;
}
...
// ffprobe 結果から撮影日時 (creation_time) を ISO(UTC) で取り出す。取れなければ null。
//   - 優先: format.tags.creation_time → 映像ストリームの tags.creation_time → 任意ストリーム
//   - "2026-09-13T10:30:12.000000Z" (iPhone / 多くの機材は UTC で記録) はそのまま UTC として解釈
//   - "2026-09-13 10:30:12" のようにタイムゾーン指定が無い値は日本時間として解釈する
//     （国内の撮影機材はカメラ時計＝現地時刻で記録することが多いため。Railway は UTC 動作なので
//       サーバーローカルに任せず +09:00 を明示する）
//   - 2000 年未満 / 未来 1 日以上先は機材の時計未設定とみなして捨てる
function _extractMediaCreatedAt(probed) {
  try {
    const cands = [];
    if (probed?.format?.tags) cands.push(probed.format.tags);
    const streams = Array.isArray(probed?.streams) ? probed.streams : [];
    for (const st of streams) if (st?.codec_type === 'video' && st.tags) cands.push(st.tags);
    for (const st of streams) if (st?.tags) cands.push(st.tags);
    for (const tags of cands) {
      const raw = tags.creation_time || tags['com.apple.quicktime.creationdate'] || tags.CREATION_TIME || null;
      if (!raw) continue;
      ...
      if (y < 2000 || d.getTime() > Date.now() + 24 * 60 * 60 * 1000) continue;
...
async function _processVideoStoryboard({ sourceDriveFileId, sourceFileName, durationSeconds, rowId, uploadAsUserDrive }) {
  ...
  try {
    ...
    // ffprobe は常に 1 回走らせる（ADR 039 D4）。
    //   - 動画長が未取得ならここで測る（全動画一律処理にしたため、短尺でも必要）
    //   - 撮影日時 (format.tags.creation_time / stream tags) を読み、振り分け時のファイル名日付に使う。
    //     原本は既にローカルにあるので追加のダウンロードは発生しない。
    let effectiveDuration = Number(durationSeconds) || 0;
    let mediaCreatedAt = null;
    const probed = await new Promise((resolve) => {
      if (!ffmpeg || !ffprobePath) return resolve(null);
      ffmpeg.ffprobe(inputFp, (err, data) => resolve((err || !data) ? null : data));
    });
    if (probed) {
      if (!effectiveDuration || effectiveDuration <= 0) {
        const d = Number(probed.format?.duration) || 0;
        if (d > 0) effectiveDuration = d;
      }
      mediaCreatedAt = _extractMediaCreatedAt(probed);
    }
    if (!effectiveDuration || effectiveDuration <= 0) {
    ...
    // 解析用プロキシ（ADR 039 D1）。原本はまだローカルにあるのでここで作る（追加ダウンロードなし）。
    const proxy = await _buildAndUploadAnalysisProxies({
      drive, uploadAsUserDrive, parentFolderId, tmpDir, frameDir: FRAME_DIR, inputFp, baseName, probed, effectiveDuration,
    });
    ...
    return {
      ...
      mediaCreatedAt,
      uploadedAsUser,
      proxy: { ...proxy, frameCount: N, startSeconds: startT, endSeconds: endT },
    };
  } catch (e) {
    return { ok: false, error: e?.message || String(e) };
  } finally {
...
async function generatePreviewForVideoOrg(arg) {
  ...
  // 撮影日時 (creation_time) は別 UPDATE で best-effort 保存する（ADR 039 D4）。
  // migration 未適用（列なし）の環境でもプレビュー完了の本体更新を巻き込まないよう分離。
  if (result.mediaCreatedAt) {
    try {
      const { error: mcErr } = await supabase.from('video_file_organization_tests')
        .update({ media_created_at: result.mediaCreatedAt })
        .eq('id', rowId);
      if (mcErr) console.warn('[preview] media_created_at 保存失敗（続行）:', mcErr.message);
    } catch (e) {
      console.warn('[preview] media_created_at 保存失敗（続行）:', e?.message || e);
    }
  }
  // 解析用プロキシの Drive ID も別 UPDATE で best-effort 保存（ADR 039 D1）。
  if (result.proxy) {
    const px = result.proxy;
    try {
      const { error: pxErr } = await supabase.from('video_file_organization_tests')
        .update({
          analysis_proxy_video_drive_file_id: px.videoDriveFileId || null,
          analysis_proxy_audio_drive_file_id: px.audioDriveFileId || null,
          analysis_proxy_audio_seconds: px.audioSeconds ?? null,
          analysis_proxy_status: px.status || null,
        })
        .eq('id', rowId);
      if (pxErr) console.warn('[preview] analysis proxy 保存失敗（続行）:', pxErr.message);
    } catch (e) {
      console.warn('[preview] analysis proxy 保存失敗（続行）:', e?.message || e);
    }
    console.log('[preview] analysis proxy:', {
      rowId, status: px.status, videoSize: px.videoSize, audioSize: px.audioSize, audioSeconds: px.audioSeconds, hasAudio: px.hasAudio, error: px.error || null,
    });
  }
  console.log('[preview] done (webp):', {
    rowId, outSize: result.previewFileSize, duration: persistedDuration,
    mediaCreatedAt: result.mediaCreatedAt || null,
