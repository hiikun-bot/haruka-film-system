...
async function _processVideoStoryboard({ sourceDriveFileId, sourceFileName, durationSeconds, rowId }) {
  ...
  try {
    ...
    for (let i = 0; i < N; i++) {
      ...
      if (rowId && (i % 5 === 0)) {
        ...
        // await しない（fire-and-forget）。frame 抽出をブロックしたくない
        _updatePreviewRow(rowId, { preview_progress_percent: pct });
      }
    ...
    if (rowId) _updatePreviewRow(rowId, { preview_progress_percent: 95 });
