...
async function triggerAutoAnalyzeIfEligible({ rowId, ignoreFeatureFlag = false }) {
  if (!rowId) {
  ...
  autoApplyLib.applyForRow({ rowId: updated.id, source: 'auto-after-analyze' })
    .catch(err => console.error('[video-org] auto-apply after analyze failed:', err?.message || err));
