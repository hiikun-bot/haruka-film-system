...
async function analyzeMedia({ mediaBuffer, mimeType, mediaKind, originalFilename, sourceVariant, originalMediaKind }) {
  ...
  // location='global' のときは `${location}-aiplatform.googleapis.com` という
  // 存在しないホスト名を叩いて HTML エラーが返るので、apiEndpoint を明示的に
  // リージョンプレフィックスなしの `aiplatform.googleapis.com` に固定する。
  // gemini-3.x preview 系は global でしか提供されていないモデルがあるため必須。
  ...
  console.info('[video-org] gemini-config', JSON.stringify({
    ...
    vertexai_nested_google_auth_library_version: getPackageVersion('@google-cloud/vertexai/node_modules/google-auth-library'),
  }));
