...
const { buildPrompt, PROMPT_VERSION, PROMPT_VERSION_PROXY } = require('./prompt');
...
// 引数:
//   mediaBuffer/mimeType      … 単一メディア（従来）
//   mediaParts [{buffer,mimeType}] … 複数メディア（ADR 039 D1: プロキシ動画 + 音声）。指定時は mediaBuffer より優先
//   modelName                 … モデル上書き（手動解析は Pro 系、ADR 039 D3）。未指定なら GEMINI_MODEL
//   proxy                     … sourceVariant='proxy-video' のときのコマ↔時刻対応情報（prompt.js 参照）
// 戻り値に usage（usageMetadata）を含める（ADR 039 D2 の費用記録用）。
//   promptText                … プロンプト本文の上書き（ADR 045: 作品ページの外部作品 AI 提案）。
//                               指定時は buildPrompt を使わず、メディア無し（テキストのみ）も許す。
async function analyzeMedia({ mediaBuffer, mimeType, mediaKind, originalFilename, sourceVariant, originalMediaKind, mediaParts, modelName: modelOverride, proxy, promptText: promptOverride }) {
  ...
  const modelName = modelOverride || guards.getModelName();
  ...
  const promptText = promptOverride || buildPrompt({
    originalFilename,
    mediaKind,
  ...
  const parts = (Array.isArray(mediaParts) && mediaParts.length ? mediaParts : [{ buffer: mediaBuffer, mimeType }])
    .filter(p => p && p.buffer)
    .map(p => ({
      inlineData: {
        mimeType: p.mimeType || (mediaKind === 'image' ? 'image/jpeg' : 'video/mp4'),
        data: p.buffer.toString('base64'),
      },
    }));
  if (!parts.length && !promptOverride) throw new Error('解析するメディアがありません');
  ...
  // ADR 039 D2: usageMetadata（prompt/candidates/thoughts/total、モダリティ別内訳）をそのまま返す
  const usage = response?.usageMetadata || null;
  try {
    console.info('[video-org] gemini-usage', JSON.stringify({ model: modelName, sourceVariant: sourceVariant || null, usage }));
  } catch (_) {}
  ...
  return {
    ...
    raw: text,
    model: modelName,
    promptVersion: sourceVariant === 'proxy-video' ? PROMPT_VERSION_PROXY : PROMPT_VERSION,
    usage,
  };
}
