...
// AUTO_APPLY_SKIP_REVIEW=true のとき、AI が needs_human_review=true と判定した素材も
// 「確認待ち (awaiting_review)」で止めずに、そのまま自動リネーム＋振り分けまで進める。
// 既定 false（安全側）。本番で完全自動運用したい場合のみ ON にする。
function isAutoApplySkipReview() {
  return truthy(process.env.AUTO_APPLY_SKIP_REVIEW);
}
