...
async function applyForRow({ rowId, userId, source = 'auto', actorUserId = null }) {
  ...
  // 確認が必要なら自動適用しない。
  // ただし AUTO_APPLY_SKIP_REVIEW=true のときは「要確認」判定でも止めず、そのまま自動適用する
  // （完全自動運用モード）。needs_human_review / reason は DB 列に残るので監査は可能。
  const skipReview = guards.isAutoApplySkipReview();
  if (item.needs_human_review === true && !skipReview) {
    await supabase.from('video_file_organization_tests')
      .update({
  ...
  if (item.needs_human_review === true && skipReview) {
    // 完全自動モード: 要確認でもこのまま下へ進めて自動リネーム＋振り分けする
    logCtx('auto-apply-review-skipped', {
      rowId: item.id, fileId: item.drive_file_id,
      confidence: item.confidence, reason: item.reason,
    });
  }
  ...
  try {
    ...
    //    user OAuth (drive.file) は SA がアップロードしたファイルを「自分が作っていない」と見て
    //    404 / insufficientFilePermissions を返すため、Hybrid フォールバックで SA でも試行する。
    if (item.preview_drive_file_id) {
      const movePreview = async (drive) => {
        const previewMeta = await drive.files.get({
        ...
        if (previewParent === finalFolderId) {
          return { moved: false, alreadyThere: true };
        }
      ...
      const isNotFoundOrForbidden = (err) => {
      ...
      try {
        await movePreview(userDrive);
      } catch (previewMoveErr) {
        if (isNotFoundOrForbidden(previewMoveErr)) {
          // SA フォールバック: SA で存在確認 → SA で移動
          try {
            const saDrive = await driveLib.getDriveService();
            const existence = await driveLib.getFile(item.preview_drive_file_id, { client: saDrive });
            if (existence.ok) {
              await movePreview(saDrive);
              console.info('[video-org] auto-apply: preview moved via SA fallback', item.preview_drive_file_id);
            } else {
              console.warn('[video-org] auto-apply: preview not found via SA either:', item.preview_drive_file_id, existence.status, existence.reason);
            }
          } catch (saErr) {
            // 致命ではないので status='applied' は維持
            console.warn('[video-org] auto-apply: preview SA fallback failed:', saErr.message);
          }
        } else {
