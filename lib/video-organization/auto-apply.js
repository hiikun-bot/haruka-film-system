...
//     2) 既に先頭 8 桁日付なら そのまま
//     3) それ以外は先頭に日付を付与
//   日付ソース（ADR 039 D4）: メディアの撮影日時 media_created_at (creation_time, JST 変換)
//     → 元ファイル名から抽出 → 取れなければアップロード日(created_at, JST)
//   iPhone の IMG_xxxx.MOV は名前に日付が無いので、後日まとめてアップロードしても
//   撮影日で名前が付くようになる。
function ensureShootDateInFilename(recommendedName, { originalFilename, createdAtISO, mediaCreatedAtISO }) {
  const name = String(recommendedName || '').trim();
  const mediaDate = mediaCreatedAtISO && !Number.isNaN(new Date(mediaCreatedAtISO).getTime())
    ? jstYYYYMMDD(mediaCreatedAtISO)
    : null;
  const date = mediaDate || extractDateFromName(originalFilename) || jstYYYYMMDD(createdAtISO);
  ...
  try {
    ...
    if (item.project_id) {
      ...
      desiredName = ensureShootDateInFilename(item.recommended_filename, {
        ...
        mediaCreatedAtISO: item.media_created_at || null,
      });
    } else {
