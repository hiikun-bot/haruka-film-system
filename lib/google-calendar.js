 ...
 * @returns {Promise<Array<{ id:string, start:string, end:string, isAllDay:boolean, status:string, transparency:string }>>}
 */
  ...
  try {
    ...
    for (let i = 0; i < 10; i++) {
      const { data } = await cal.events.list({
        ...
        fields: 'nextPageToken,items(id,status,transparency,start(dateTime,date,timeZone),end(dateTime,date,timeZone))',
      });
