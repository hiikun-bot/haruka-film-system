...
//
// 🔒 プライバシー原則（全フェーズ共通・厳守）:
//   Google Calendar から取得するのは「時間情報のみ」。予定の内容
//   （summary / description / location / attendees / creator / organizer /
//    conferenceData / attachments など）は API リクエストの段階で
//   `fields` パラメータにより取得しない・保存しない・ログ出力しない。
//   後段フェーズで DB に保存する `gcal_raw_slots` 等にも内容は一切入れない。
//
// 必要な環境変数:
 ...
 * 指定範囲の予定を取得する。
 *
 * 🔒 プライバシー: `fields` パラメータで時間関連のみに送信フィールドを制限。
 *   summary / description / location / attendees / creator / organizer /
 *   conferenceData / attachments 等の「内容」は API レスポンスに含まれない。
 *
 * @param {object} args
 ...
 * @returns {Promise<Array<{ id:string, start:string, end:string, isAllDay:boolean, status:string, transparency:string, eventType:string }>>}
 */
  ...
  try {
    ...
    for (let i = 0; i < 10; i++) {
      const { data } = await cal.events.list({
        ...
        maxResults: 250,
        pageToken,
        // 🔒 プライバシー保護: 取得フィールドを時間情報のみに制限
        //   id           : 同期判定用
        //   start / end  : 時間情報（dateTime / date / timeZone を含む）
        //   status       : cancelled 除外用
        //   transparency : transparent（予定なし扱い）除外用
        //   eventType    : 勤務場所/誕生日/不在 など終日イベントの種別判定用（件名等の中身ではない）
        //   nextPageToken: ページング継続用
        fields: 'nextPageToken,items(id,status,transparency,eventType,start(dateTime,date,timeZone),end(dateTime,date,timeZone))',
      });
