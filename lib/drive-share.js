...
async function shareForClientReview({ creativeId, force = false } = {}) {
  ...
  // 2. 関連 creative_files から「最新バージョン」を1件選ぶ（pickPrimaryFile 内で version DESC）
  const file = await pickPrimaryFile(creativeId);
  if (!file) {
    // 最新ファイルが取れず、既存URLだけある（手動入力等）→ 既存を尊重
    if (creative.client_review_url && String(creative.client_review_url).trim()) {
  ...
  // 既存 client_review_url が最新ファイルを既に指していれば skip（idempotent）。
  // 古いバージョン（v1）を指したまま v2 提出時に更新されないバグ #97cac31d への対応:
  //   webViewLink は通常 `https://drive.google.com/file/d/<fileId>/view...` 形式で fileId を含む。
  //   この fileId 部分が最新の targetFileId と一致しないなら「古い」と判定して force update。
  if (!force && creative.client_review_url) {
    const existingUrl = String(creative.client_review_url).trim();
    if (existingUrl && existingUrl.includes(targetFileId)) {
      return { url: existingUrl, source: 'existing' };
    }
    // existingUrl が最新ファイルを指していない → 旧バージョンの自動共有 or 手動URL。
    // 修正版バグ #97cac31d の主因: 既存URLが v1 のまま残るのを防ぐため override する。
  }
  ...
  // 5. race condition 対策で再取得 → 既に値があれば（最新ファイルを指している場合のみ）尊重
  if (!force) {
    const { data: again } = await supabase
      ...
      .eq('id', creativeId)
      .maybeSingle();
    const againUrl = again?.client_review_url && String(again.client_review_url).trim();
    if (againUrl && targetFileId && againUrl.includes(targetFileId)) {
      return { url: againUrl, source: 'existing' };
    }
  }
...
module.exports = {
  ...
  // ensureAnyoneReader は他ルート（クリエイティブの Drive フォルダ一括DL等）でも
  // 同じ idempotent な anyone-with-link reader 付与を流用するため公開する。
  ensureAnyoneReader,
};
