...
-- ============================================================
-- ADR 017 Phase 0: メンバー稼働時間プロフィール（基本枠 + GCal 接続）
--   migration: 2026-05-14_member_working_hours_phase0.sql
--   日次テーブル (_daily) は Phase 1 で追加
-- ============================================================
...
CREATE TABLE IF NOT EXISTS member_working_hours_profile (
  ...
  gcal_refresh_token_encrypted text,
  gcal_token_iv     text,
  gcal_token_auth_tag text,
...
CREATE TABLE IF NOT EXISTS personal_tasks (
  ...
  link_url TEXT,               -- 資料URL
  due_date DATE,
  priority TEXT CHECK (priority IN ('top','high','mid','low')),  -- 優先度（任意。top=最優先だけ画面に赤表示）
  assignee_user_ids UUID[],    -- 担当メモ（複数名・非共有。相手には見えない自分用メモ・FK無し）
  status TEXT NOT NULL DEFAULT '未着手' CHECK (status IN ('未着手','対応中','相手待ち','予約済み','完了')),
  is_milestone BOOLEAN NOT NULL DEFAULT false,  -- 節目の日（例: 会社設立日）。ステータスの代わりにマイルストーン表示
...
-- ============================================================
-- 📝 契約管理（ADR 035）
-- 詳細: migrations/2026-09-07_contracts.sql / docs/design/decisions/035-contract-management.md
-- ============================================================
-- ============================================================
-- 1) 契約主体マスタ（自社情報）
-- ============================================================
