...
| 案件 line / 単価 (Stage 1+) | `project_estimate_lines` / `project_estimate_line_costs` / `project_fixed_items` | `routes/haruka.js` の `/api/projects/:id/lines` (Stage 4a) / `utils/pricing.js` | `public/haruka.html` 案件モーダル「成果物グループ」セクション（Stage 4a） | ADR 002+005+006。lines は見積行 兼 deliverable。`status IN ('contracted','in_progress','delivered')` のみ集計対象。line_costs/fixed_items の CRUD UI は Stage 4b/4c で追加予定。旧 rates 系（director-rates / producer-rates / client-fee）は Stage 4d で整理予定 |
