...
| 💸 振込管理（月次振込の消し込み） | `payout_records`（user×年月・permission `payout.page`） | `routes/haruka.js` の `/api/haruka/admin/payouts*` + `utils/payout.js`（定型文/金額抽出/差分判定の純関数・jestテストあり） | `public/haruka.html` 独立タブ `#page-payout` + `loadPayoutPage()`（ヘッダーナビ「💸 振込管理」・admin のみ表示） | Drive請求書フォルダ（請求書/YYYY年/MM月/氏名…）を開くたび再スキャンし新PDFを検出。PDF→Gドキュメント変換で請求額を自動抽出（失敗時は手入力）。行ごとに「振込完了→送信」で Chatwork 定型文＋任意メモを本人へ通知し緑で消し込み。「差分チェック」で本人 per-unit 実データ（resolveCreativeRoleCost）と請求額を突合 |
