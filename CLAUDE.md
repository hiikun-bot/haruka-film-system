...
| 🎬 新着納品ショーケース / showcase / スライドショー / まとめてナイス！ / showcase_hidden / 既読はlocalStorage | [decisions/042](docs/design/decisions/042-portfolio-reactions.md)（追補 2026-09-17） |
...
| 🎬 新着納品ショーケース（直近 7 日の納品作品をホームでスライドショー表示し、その場で 👏） | `projects.showcase_hidden`（機密案件の除外・既定 false）。既読・畳み状態は DB に持たず localStorage（`hfs_showcase_seen` / `hfs_showcase_collapsed`） | `routes/haruka.js` の `GET /api/haruka/showcase?days=7`（requireAuth・作品ページと同じ認可。`fetchPortfolioFilesByCreative` / `resolvePortfolioAspect` / `fetchPortfolioSocialMap` を作品一覧と共用）+ `POST /portfolio/reactions/bulk`（まとめてナイス！・通知は宛先 1 人 1 通）+ `utils/showcase.js`（JST 日付境界・4 本以上のまとめ判定 `bundleDeliveries` の純関数・jestテストあり） | `public/haruka.html` ホーム `#dash-showcase` + `renderShowcaseCard()`（`loadDashboard()` から全ロール。置き場だけ `effectiveRole()` で移す）+ 全画面スライドショー `#showcase-overlay`（`scOpenSlideshow` / `scRenderSlide`）。リアクション行は作品ページの `pfReactRowHtml` / `pfToggleReaction` を ctx `'sc'` / `'sccard'` で再利用（`pfItemsFor(ctx)`）。案件編集モーダルの「🎬 ショーケースに出さない」 | ADR 042 追補（2026-09-17）。祝う場なので修正回数・遅延・単価・ランクは返さない・出さない。ホームを開いた瞬間に自動再生しない。同一案件 × 同一納品日（JST）4 本以上は 1 スライドにまとめる。つぶやきへの自動投稿はしない（#1182 の 🔗 つぶやきで紹介 と重複するため）。新着ゼロの週はカードを出さない |
