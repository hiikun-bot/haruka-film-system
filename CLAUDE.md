...
| 費用台帳同期 | `project_estimate_lines` / `project_estimate_line_costs` / `project_director_rates` / `clients.billing_org` / `system_settings.cost_ledger_sheet_url` | `routes/haruka.js` の `/api/cost-ledger/*` + `utils/cost-ledger-sync.js` | `public/haruka.html` 管理画面（page-master）「📊 費用台帳同期」`renderCostLedgerSyncPage()` | ADR 024。スプレッドシートのタブ「費用台帳」と双方向同期（1行=1見積行＋非表示ID列）。エクスポート=DB→シート / インポート=差分プレビュー→反映。admin/secretary 限定 |
