...
## 追記（2026-08-21）: Phase 2 L3/L4 の表示面を廃止

マイタスクパネル（ヘッダー ✅ アイコン + スライドパネル）と、ダッシュボードの
「✅ あなたのタスク」「🎯 今週の山場」の 2 カードは**未使用のため廃止**（PR #1066）。
専用 API（`GET /my-tasks`・`GET /my-tasks/count`・`PATCH /my-tasks/:task_id/done`・
`GET /dashboard/upcoming-milestones`）も併せて削除した。

**案件スケジュール編集機能本体（`project_tasks` テーブル・案件モーダル内の工程表 UI・
`/api/projects/:id/tasks` CRUD・フェーズテンプレ）は継続**。ADR 016 の拡張も影響なし。
