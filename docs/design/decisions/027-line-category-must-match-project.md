...
## 実施結果（2026-07-02 本番コンバート完了）

診断時点で全45案件に主カテゴリ設定済み（制限が効かない案件ゼロ）。
不一致 line は 48 件（全件「旧 project_rates 移行」由来）で、以下のとおり処理した。

- **45 件削除**: 未使用（creatives.line_id 参照 0・invoice_items.line_id 参照 0）。
  line_costs ごと削除。`deleted_lines = 45` を本番で確認済み
- **3 件温存**（納品済みクリエイティブ計4本の単価根拠のため削除・停止どちらも不可。
  停止 status にすると resolveCreativeRoleCost の status フィルタで直接紐付けが
  空になり、過去の支払い集計・明細の単価が 0 円に化ける）:
  - サムネイル（静止画）× 動画Bランク — video_short 1本（2026-05-06 納品）
  - ショート動画（動画）× 静止画Bランク — design_thumbnail 1本（2026-05-04 納品）
  - 静止画（静止画）× 動画Aランク — video_short / design_post 計2本（2026-05-03 納品）

温存 3 件は本 ADR の完全ブロックにより今後増殖しない。実態どおりの混在制作実績
（2026年5月上旬・移行期）であり、今後同種の仕事は案件を分けて登録する。
