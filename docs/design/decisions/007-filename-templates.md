...
### マスター引用（既存 system トークンの位置づけ）

ファイル名に「マスタから選ばれた値」を入れる需要は、すべて **system トークンで賄う**:
- 商品マスタ → `product` トークン
- 訴求軸マスタ → `appeal_axis` トークン
- サイズ / 媒体 / フォーマット / クライアントコード / 日付 → 各 system トークン

将来、新たなマスタ（例: クライアント別カスタムマスタ、季節マスタ等）から引用したくなったときは、新しい system トークンを1つ追加し、`utils/filename.js` の値解決マップに 1 行追加するだけで対応する。kind を増やさずに拡張可能。

`kind: "custom"`（テキスト固定値）は当面のメイン用途を flag 型に譲るが、アドホックなラベル差し込みのため残す。テンプレ管理 UI のトークン追加モーダルでは「マスター引用 / フラグ / テキスト」の3択で提示する（マスター引用は既存 system トークンのピッカー）。
