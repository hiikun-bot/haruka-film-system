...
## 補足: 動画ファイルなしで工程を次へ進める運用（2026-05-09 / 代表 髙橋指示）

### 背景
- 動画はクライアントへ Slack DM 等で直接共有済み（システム上はファイル不要）
- 修正対応が口頭・他チャネルで完結し、ファイルを差し替えずに工程だけ進めたい
- 素材待ちで動画が作れていないが工程は進めたい（レアケース）

### 仕様

1. **ボタン活性化**: 動画ファイル未アップロード状態でも「Dチェックへ提出 →」「Pチェックへ提出 →」等の各種「次へ／提出」ボタンを押下可能とする。disabled は付けない。
2. **確認ダイアログ**: ファイル未アップロードの状態で押下した場合のみ、専用の「動画ファイルなしで進めますか？」ダイアログを表示。理由テキストは任意入力（プレースホルダ例: `クライアント直接やり取り済み / 口頭で完結 / 素材待ち`）。
3. **API**: `PUT /api/creatives/:id` の body に `fileless_progression: true` と `fileless_reason: <string>` を追加で送る。サーバ側はファイル必須バリデーションを持っていないため、追加バリデーションは無し。
4. **snapshot**: 既存の「修正済 → 再チェック」遷移時 snapshot ロジックは維持しつつ、`fileless_progression=true` の場合に以下の差を入れる:
   - `creative_file_id = NULL`
   - `version_num = max(MAX(snapshot.version_num), MAX(creative_files.version)) + 1`（次のラウンドへ進める）
   - `editor_comment` の先頭にプレフィックス `[動画なし] <reason>\n` を付与（reason 空なら `[動画なし]` のみ）
5. **ラウンド比較 UI（左カラム下部）**: 該当ラウンドではファイルカードの代わりに琥珀色の「📋 動画ファイルなし提出（バージョン N）／理由: ◯◯」プレースホルダを描画。3層構造（タグ／カード／余白）の高さは保つ。
6. **取り消し**: 動画なし提出も「提出済み」扱いとし、取り消し不可。snapshot に乗っている時点で PR #439 のルール踏襲。
7. **バージョン番号**: ラウンド = `version_num` のルール（PR #439）は維持。動画なし提出でラウンドは進むが `creative_files` の行は作らない。

### Consequences
- ファイル必須運用が崩れるが、現場運用に合わせる（ファイル不要なケースが恒常的に発生）。
- snapshot に必ず履歴が残るため、後から「動画なしで通したラウンド」が監査できる。
- `creative_files` と `creative_version_history.version_num` が同期しなくなるラウンドが生じるが、`creative_file_id` がフォールバック解決の主キーになっているため UI の整合は保たれる。
