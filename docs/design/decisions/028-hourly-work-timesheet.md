...
## 改訂（2026-07-06 ユーザー指示）— 「時間給作業」案件カテゴリの新設

**時給で発生する案件を、案件カテゴリ「時間給作業」(code='hourly') として作れるようにする。**
これまで時給案件にするには案件収支の「⏱ ディレクター費（時給）」を設定する必要があり
導線が奥まっていた。新規案件モーダルのカテゴリ選択から一発で入れるようにする。

- `creative_categories` に render_kind='timesheet'（成果物なし）を追加し、
  code='hourly' / name='時間給作業' を投入
  （migration: `migrations/2026-07-06_hourly_work_category.sql`）
- 新規/編集モーダルで「時間給作業」を選ぶと、案件ごとの**支払時給・請求時給**入力欄を表示。
  保存時に既存の `PUT /projects/:id/director-fee`（hourly モード）を呼び、
  時間制グループ（`pricing_type='hourly'` の line_cost）を自動作成/更新する
  → 単価スナップショット・集計・請求プレビューは第2回改訂までの仕組みをそのまま流用
- `GET /work-hours/projects` は、単価行が未設定でも primary_category.code='hourly' の案件を
  `has_hourly=true` として先頭に並べる（「選ぶだけで作業時間画面に出る」を担保）
- 請求時給の入力は `project.client_price` 権限を持つロールのみ表示（ADR 015・サーバー側でも同権限でガード）
- 時間給作業カテゴリの案件は成果物を持たない前提（案件詳細はほぼ使わず、稼働は⏱作業時間報告で入力）。
  成果物が必要な仕事は別カテゴリの案件で作る（ユーザー確認済み）
